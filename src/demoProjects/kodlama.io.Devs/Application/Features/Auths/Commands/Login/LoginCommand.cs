...
namespace Application.Features.Auths.Commands.Login
{
    public class LoginCommand:IRequest<LoginDto>
    {
        ...
        public class LoginCommandHandler : IRequestHandler<LoginCommand, LoginDto>
        {
            ...
            {
                await _authBusinessRules.EmailShouldBeExistInTheUserTableWhenUserTryingToLogin(request.UserForLoginDto.Email);

                User? user = await _userRepository.GetAsync(u => u.Email.ToLower() == request.UserForLoginDto.Email.ToLower());

                await _authBusinessRules.PasswordShouldBeValidWhenUserTryingToLogin(request.UserForLoginDto.Password, user.PasswordHash, user.PasswordSalt);
